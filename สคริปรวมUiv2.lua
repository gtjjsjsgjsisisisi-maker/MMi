...
-- ระบบชนผู้เล่นกระเด็น
RunService.Heartbeat:Connect(function()
    if flingEnabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        LocalPlayer.Character.HumanoidRootPart.AssemblyAngularVelocity = Vector3.new(0, 999999, 0)
    end
end)
...
local function startFlying()
    ...
    flyConnection = RunService.RenderStepped:Connect(function()
        ...
        if moveVector.Magnitude > 0 then velocity = moveVector * flySpeed end
        if flyUp then velocity = velocity + Vector3.new(0, flySpeed, 0) end
        if flyDown then velocity = velocity - Vector3.new(0, flySpeed, 0) end
...
LocalPlayer.CharacterAdded:Connect(function(character)
    ...
    if humanoid then
        task.wait(0.1)
        if speedEnabled then humanoid.WalkSpeed = fastSpeed end
        if invisibleEnabled then setInvisibility(true) end
        if flyEnabled then
            stopFlying()
            if flyToggleObj then flyToggleObj:SetValue(false) end
        end
    end
end)
...
MainTab:Section({ Title = "ข้อมูลตัวละคร & สถานะ" })

-- การ์ดแสดงผลโปรไฟล์แบบเนทีฟ WindUI
local ProfileParagraph = MainTab:Paragraph({
...
-- อัปเดตข้อมูล FPS, เลือด, ชื่อแมป Real-time
task.spawn(function()
    while task.wait(0.2) do
        pcall(function()
            ...
            ProfileParagraph:SetTitle(LocalPlayer.DisplayName .. " (@" .. LocalPlayer.Name .. ")")
            ProfileParagraph:SetDesc(
                "⚡ FPS: " .. currentFPS ..
                "  |  ❤️ Health: " .. currentHp .. " / " .. maxHp ..
                "\n🗺️ Map: " .. mapName ..
                "\n👥 Players: " .. #Players:GetPlayers() .. " คน  (ID: " .. game.PlaceId .. ")"
            )
        end)
    end
end)
...
MainTab:Button({
    ...
    Callback = function()
        playSound(SOUND_CLICK)
        pcall(function() setclipboard("https://www.instagram.com/your_instagram") end)
    end
})
...
MainTab:Button({
    ...
    Callback = function()
        playSound(SOUND_CLICK)
        pcall(function() setclipboard("https://discord.gg/your_discord") end)
    end
})
...
MainTab:Button({
    ...
    Callback = function()
        playSound(SOUND_CLICK)
        pcall(function() setclipboard("https://www.tiktok.com/@your_tiktok") end)
    end
})
...
VisualsTab:Toggle({
    ...
    Callback = function(Value)
        ...
        if not Value then updatePlayerEsp() end
    end
})
...
VisualsTab:Toggle({
    ...
    Callback = function(Value)
        playSound(SOUND_CLICK)
        npcEspEnabled = Value
        if not Value then updateNpcEsp() end
    end
})
